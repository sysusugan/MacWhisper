import Foundation

struct HotkeyConfig: Codable, Equatable {
    /// The hold key — hold to record, release to stop
    var holdKey: KeyCombo
    /// The toggle combo — press to lock on, press again to stop
    var toggleCombo: KeyCombo

    static let defaultConfig = HotkeyConfig(
        holdKey: KeyCombo(keyCode: nil, modifiers: KeyModifiers(fn: true)),        // Hold fn
        toggleCombo: KeyCombo(keyCode: 49, modifiers: KeyModifiers(option: true))  // Option + Space
    )
}

struct KeyCombo: Codable, Equatable {
    var keyCode: UInt16?  // nil = modifier-only
    var modifiers: KeyModifiers

    var displayString: String {
        var parts: [String] = []
        if modifiers.control { parts.append("⌃") }
        if modifiers.option { parts.append("⌥") }
        if modifiers.shift { parts.append("⇧") }
        if modifiers.command { parts.append("⌘") }
        if modifiers.fn { parts.append("fn") }
        if let kc = keyCode {
            parts.append(keyCodeToString(kc))
        }
        return parts.isEmpty ? "Not Set" : parts.joined(separator: " ")
    }
}

struct KeyModifiers: Codable, Equatable {
    var control: Bool = false
    var option: Bool = false
    var shift: Bool = false
    var command: Bool = false
    var fn: Bool = false

    var isEmpty: Bool {
        !control && !option && !shift && !command && !fn
    }
}

func keyCodeToString(_ keyCode: UInt16) -> String {
    let map: [UInt16: String] = [
        0: "A", 1: "S", 2: "D", 3: "F", 4: "H", 5: "G", 6: "Z", 7: "X",
        8: "C", 9: "V", 11: "B", 12: "Q", 13: "W", 14: "E", 15: "R",
        16: "Y", 17: "T", 18: "1", 19: "2", 20: "3", 21: "4", 22: "6", 23: "5",
        24: "=", 25: "9", 26: "7", 27: "-", 28: "8", 29: "0", 30: "]", 31: "O",
        32: "U", 33: "[", 34: "I", 35: "P", 36: "Return", 37: "L", 38: "J",
        39: "'", 40: "K", 41: ";", 42: "\\", 43: ",", 44: "/", 45: "N", 46: "M",
        47: ".", 48: "Tab", 49: "Space", 50: "`", 51: "Delete", 53: "Esc",
        96: "F5", 97: "F6", 98: "F7", 99: "F3", 100: "F8", 101: "F9",
        103: "F11", 105: "F13", 107: "F14", 109: "F10", 111: "F12",
        113: "F15", 115: "Home", 116: "PgUp", 117: "Fwd Del", 118: "F4",
        119: "End", 120: "F2", 121: "PgDn", 122: "F1", 123: "←", 124: "→",
        125: "↓", 126: "↑",
    ]
    return map[keyCode] ?? "Key \(keyCode)"
}
